      ...
      <LangCodeProvider code={pageContext.wpmlLang}>
        ...
        >
          ...
          <Helmet>
            <link 
              rel="canonical" 
              href={`${process.env.GATSBY_SITE_DOMAIN}${pageContext.prefix}privacy-policy`} 
            />
            { pageContext.langList.map(lang => {
              return (
                <link 
                  ...
                  href={`${process.env.GATSBY_SITE_DOMAIN}${lang.path}privacy-policy`} 
                />
              )
            }) }
            <link 
              rel="alternate" 
              hreflang="x-default"
              href={`${process.env.GATSBY_SITE_DOMAIN}/privacy-policy`} 
            />
          </Helmet>
          <Breadcrumbs className="privacy-breadcrumbs" list={breadcrumbs} />
...
export const query = graphql`
  query getPrivacyPolicyPage($lang: String) {
    wpPage(locale: {locale: {eq: $lang}}, slug: {eq: "privacy-policy"}) {
      title
      uri
      seo {
        title
