...
const Delivery = ({ pageContext, data }) => {
  // Meta Data 
      ...
      <LangCodeProvider code={pageContext.wpmlLang}>
        ...
        >
          ...
          <Helmet>
            <link 
              rel="canonical" 
              href={`${process.env.GATSBY_SITE_DOMAIN}${pageContext.prefix}usloviya-zakaza`} 
            />
            { pageContext.langList.map(lang => {
              return (
                <link 
                  ...
                  href={`${process.env.GATSBY_SITE_DOMAIN}${lang.path}usloviya-zakaza`} 
                />
              )
            }) }
            <link 
              rel="alternate" 
              hreflang="x-default"
              href={`${process.env.GATSBY_SITE_DOMAIN}/usloviya-zakaza`} 
            />
          </Helmet>
          <Breadcrumbs className="delivery-breadcrumbs" list={breadcrumbs} />
...
export const query = graphql`
  query getDeliveryPage($lang: String) {
    wpPage(locale: {locale: {eq: $lang}}, slug: {eq: "usloviya-zakaza"}) {
      title
      uri
      seo {
        title
