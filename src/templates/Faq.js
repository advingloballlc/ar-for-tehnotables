...
const Faq = ({ pageContext, data }) => {
  ...
  const { faqPageFields, questionFormFields } = data.wpPage
      ...
      <LangCodeProvider code={pageContext.wpmlLang}>
        ...
        >
          ...
          <Helmet>
            <link 
              rel="canonical" 
              href={`${process.env.GATSBY_SITE_DOMAIN}${pageContext.prefix}faq`} 
            />
            { pageContext.langList.map(lang => {
              return (
                <link 
                  ...
                  href={`${process.env.GATSBY_SITE_DOMAIN}${lang.path}faq`} 
                />
              )
            }) }
            <link 
              rel="alternate" 
              hreflang="x-default"
              href={`${process.env.GATSBY_SITE_DOMAIN}/faq`} 
            />
          </Helmet>
          <Breadcrumbs className="faq-breadcrumbs" list={breadcrumbs} />
          <FaqIntro 
            title={pageTitle} 
            faqPageFields={faqPageFields} 
            questionFormFields={questionFormFields} 
            validateErrors={validateErrors}
          />
          <ModalThanks
            desc={faqFormSuccess.mainText} 
            btn={faqFormSuccess.btnText}
          />
          <ModalError
...
export const query = graphql`
  query getFaqPage($lang: String) {
    wpPage(locale: {locale: {eq: $lang}}, slug: {eq: "faq"}) {
      title
      uri
      seo {
        title
